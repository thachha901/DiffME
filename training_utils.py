...
def spotting(final_samples, subject_count, pred_interval, total_gt_spot, 
             p, metric_final, k_p,
             max_events_per_video=None):   # NEW: giới hạn số event / video (None = như cũ)
    pred_subject = []
    gt_subject = []
    ...
    for videoIndex, video in enumerate(final_samples[subject_count]):
        ...
        # -------- 1) lấy score + smoothing (y như code gốc) --------
        score_plot = np.array(pred_interval[videoIndex])
        score_plot = smooth(score_plot, k_p * 2)          
        score_plot_agg = score_plot.copy()

        # -------- 2) Moilanen threshold (y như code gốc) --------
        mu = score_plot_agg.mean()
        mx = max(score_plot_agg)
        threshold = mu + p * (mx - mu)                    # y như trước

        # -------- 3) tìm peak (y như code gốc, chỉ thêm property để lấy height) --------
        peaks, props = find_peaks(score_plot_agg,
                                  height=threshold,
                                  distance=k_p)
        peak_heights = props.get("peak_heights", np.array([]))

        # -------- 4) nếu muốn, giới hạn top-K peak theo height --------
        # giữ lại behaviour cũ nếu max_events_per_video = None
        if max_events_per_video is not None and len(peaks) > 0:
            # list [(peak_index, height), ...]
            peaks_with_scores = list(zip(peaks.tolist(), peak_heights.tolist()))
            # sort theo height giảm dần
            peaks_with_scores = sorted(
                peaks_with_scores, key=lambda x: x[1], reverse=True
            )
            # chỉ giữ K peak cao nhất
            peaks_with_scores = peaks_with_scores[:max_events_per_video]
        else:
            peaks_with_scores = list(zip(peaks.tolist(), peak_heights.tolist()))

        T = len(score_plot_agg)

        # -------- 5) tạo preds từ peaks (giống cũ, chỉ thêm clip biên) --------
        for peak, h in peaks_with_scores:
            peak = int(peak)
            onset = max(0, peak - k_p)          # NEW: clip biên trái
            offset = min(T - 1, peak + k_p)     # NEW: clip biên phải

            # format giữ y nguyên (score vẫn = 0 để tránh ảnh hưởng mAP)
            preds.append([onset, 0, offset, 0, 0, 0, peak])

        # nếu không có peak nào
        if len(peaks_with_scores) == 0 or len(preds) == 0:
            preds_np = np.empty((0, 7))
        else:
            preds_np = np.array(preds)

        # -------- 6) ground truth (giữ nguyên) --------
        for samples in video:
            gt.append([samples[0], 0, samples[2], 0, 0, 0, 0, samples[1]])
            total_gt_spot += 1
        gt_np = np.array(gt)

        # -------- 7) cập nhật metric (giữ nguyên) --------
        metric_video.add(preds_np, gt_np)
        metric_final.add(preds_np, gt_np)

        # lưu lại theo video cho recognition
        pred_subject.append(preds)   # giữ kiểu list-of-lists như code gốc
