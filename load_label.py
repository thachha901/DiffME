...
def load_label(dataset_name, images, subjects, subjectsVideos, codeFinal, frame_skip):
    ...
    # Debug: In ra 5 dòng đầu của Excel
    print("\n=== DEBUG Excel Data ===")
    print("First 5 rows of codeFinal:")
    print(codeFinal.head())
    print("\nColumn names:", codeFinal.columns.tolist())
    print("\nSample subjectCode values:", codeFinal['subjectCode'].head(5).tolist())
    print("Sample videoCode values:", codeFinal['videoCode'].head(5).tolist())
    print("Sample type values:", codeFinal['type'].unique())

    print("\n=== DEBUG Loaded Data ===")
    print("Subjects from load_images:", subjects[:5])
    print("SubjectsVideos sample:", subjectsVideos[0][:3] if len(subjectsVideos) > 0 else "Empty")
    for sub_video_each_index, sub_vid_each in enumerate(subjectsVideos):
        ...
        for videoIndex, videoCode in enumerate(sub_vid_each):
            ...
            for i, row in codeFinal.iterrows():
                subject_from_excel = str(row['subjectCode']).split('.')[-1]
                subject_from_loaded = str(subjects[sub_video_each_index])
                
                if (subject_from_excel == subject_from_loaded):
...
def cal_k_p(dataset_name, final_samples):
    ...
    if len(samples) == 0:  # Thêm kiểm tra này
        print("Warning: No samples found! Returning default k_p=0")
        return 0
