...
def main(config):
    ...
    if flow_process:
        ...
        if dataset_name == "CASME_3":
            ...
            for i in range(5):
                if method_type == 1:
                    final_dataset_spotting.extend(pickle.load( open("cache/DiffME/"+dataset_name + "_dataset_" + str(i+1) +".pkl", "rb" ) )) # CASME_3 is too large and is divided into five parts
                # else:
                #     final_dataset_spotting.extend(pickle.load( open("cache/DiffME+/"+dataset_name + "_dataset_" + str(i+1) +".pkl", "rb" ) )) 
        else:
            if method_type == 1:
                final_dataset_spotting = pickle.load( open("cache/DiffME/"+dataset_name + "_dataset.pkl", "rb" ) )
            # else:
            #     final_dataset_spotting = pickle.load( open("cache/DiffME+/"+dataset_name + "_dataset.pkl", "rb" ) )
