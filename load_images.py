...
def load_images(dataset_name, frame_skip):
  ...
  # For dataset SAMMLV
      ...
      for i, dir_vid in enumerate(natsort.natsorted(glob.glob("./SAMMLV/SAMM_longvideos/*"))): 
...
def load_information(dataset_name, frame_skip):
  ...
  # For dataset SAMMLV
      ...
      for i, dir_vid in enumerate(natsort.natsorted(glob.glob("./"+ dataset_name + "/SAMM_longvideos/*"))): 
        subject = dir_vid.split('/')[-1].split('_')[0]
        if (subject not in subjects): #Only append unique subject name
          subjects.append(subject)
          subjectsVideos.append([])
